...
**Why smolagents `ToolCallingAgent` (not `CodeAgent`)?**
The task involves structured database operations with predictable tool signatures. `ToolCallingAgent` keeps the agent in a tighter call loop and prevents it from generating arbitrary Python, which reduces the risk of unintended database mutations during exploration.

**Why append-only transactions instead of mutable stock columns?**
The `transactions` table records every stock event with a date. This means stock levels and cash balances can be reconstructed at any historical point, the audit trail is never destroyed, and test scenarios that replay dates are fully reproducible without resetting the database.

**Why filter `total_amount = -1` in quote history?**
Approximately 5–6% of rows in `quotes.csv` contain parsing errors (`-1` totals). Including them as pricing references would cause the QuoteAgent to anchor on $-1 and produce nonsensical outputs, so they are excluded before the results are returned to the agent.

**Why pass `event_type`, `order_size`, and `job_type` separately into `handle_request`?**
The quote history search is most effective when it can match on all three dimensions simultaneously. Embedding them in the free-text request string risks the LLM omitting them when constructing search keywords; passing them as explicit parameters makes the routing prompt deterministic.
